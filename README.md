...
- 🧠 **context% 계산**: Codex rollout의 token usage를 읽어 현재 context-window 사용률을 계산합니다.
- 📊 **5H / 7D usage bar**: 5시간, 7일 rate limit 사용률과 reset 시간을 함께 보여줍니다.
- 🌿 **git 상태 표시**: 현재 branch, dirty count, clean 상태를 HUD에 표시합니다.
...
터미널에서는 색상이 적용됩니다. GitHub README에서는 색상이 보이지 않을 수 있습니다.

```text
  🤖 gpt-5.5 ⚡xhigh ✅ clean no env
  📂 /Users/wsb/Documents/Github/codex-vitals 🌿(main) 🧾 2.1M tokens ⏰ 42m
...
stock Codex는 system prompt, tools, `/compact` 여유 공간을 위해 baseline `12000` tokens를 예약합니다. 그래서 기본 `codex` 모드는 `total_tokens`와 `model_context_window`에서 각각 `12000`을 차감합니다.

현재 사용 중인 HUD와 숫자가 맞지 않는다면 patched Codex HUD 공식을 쓰고 있을 가능성이 큽니다. 이 경우 아래처럼 실행하세요.
...
```bash
codex-vitals -once -rollout /path/to/session.jsonl -style answer-footer --no-color
```
...
`answer-footer` 스타일은 현재 HUD와 맞추기 위해 usage limit을 사용률 기준으로 보여줍니다. `current-hud` 단일 라인은 남은 비율을 함께 확인하기 쉽게 `left` 형태로 표시합니다.
...
`codex-vitals`는 Codex CLI용 독립 Go 구현입니다. HUD 구성과 20-block gradient bar의 시각적 방향은 terminal statusline 계열 도구에서 영감을 받았습니다.

<sub>Design reference: <a href="https://github.com/AwesomeJun/CC-statusline">AwesomeJun/CC-statusline</a>. No source code is copied.</sub>
